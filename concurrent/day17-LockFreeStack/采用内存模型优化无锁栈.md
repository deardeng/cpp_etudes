...
答案是否定的，单从线程角度b和c并不能构成同步，但是b和c必然有一个线程先执行一个线程后执行fetch_sub(原子变量的操作任何顺序模型都能保证操作的原子性)。假设b先执行，和a构成release-sequence关系，b读取到a执行的count strore的结果，那a syncronizes-with b. b处于以a线程的release为首的释放序列中，则b的store操作会和c的读-改-写(fetch操作)构成同步(c 采用的是acquire). [C++并发编程一书](https://book.douban.com/subject/35653912/)中对类似的代码也做了同样的解释。
...
1 a线程和b线程构成release-sequence的释放序列，且a synchronizes-with b。
...
1 push 采用release内存序修改head

2 pop 采用acquire内存序修改head

3 push和pop构成同步关系，形成release-sequence

4 第一个pop的线程的写操作和之后的pop线程读操作构成同步关系，形成release-sequence。
...
我们先讨论increase_head_count的实现,因为我们在increase_head_count的时候很可能其他的线程执行push操作。为保证对正确的头部增加引用计数，所以increase_head_count内部要采用acquire的方式进行读改写，和push构成release-sequence
...
1  如果head和old_head相等则说明本线程抢占了head并且需要对外部引用计数-2，得出其他线程增加的引用计数，如果这个引用计数为内部的引用计数的负数则说明其他线程已经不再占有head，已经做了内部引用计数的更新，本线程回收资源即可。
...
1 如果线程a push 操作先于b线程和c线程的pop中的increase_count操作，那么b线程2处的compare_exchange_strong不需要和c线程2处的compare_exchange_strong同步。b线程和c线程的compare_exchange_strong操作都处于释放序列，他们都能看到head的最新值。绘制运行图，实线标识先行，虚线表示释放序列

![https://cdn.llfc.club/1703733885663.jpg](https://cdn.llfc.club/1703733885663.jpg)


所以总上所述比较交换采用relaxed即可，大家不放心可以采用acquire方式。

接下来考虑compare_exchange_strong比较成功和失败之后各自内部的逻辑，因为我们要保证ptr的data在被删除之前swap到res里。所以我们可以在if逻辑（代码4处）修改为release内存存序列，在else的fetch_sub修改为acquire顺序，这样能保证swap 先行于 else处的delete.
